  ...
  return data
    .map((item) => {
      ...
      if (status === "paid") {
        ...
        const allPaid = item.installments.every((inst) => inst.status === "paid");
...
console.log(activeTab, "activeTab");
  ...
  const filteredInstallments = filterInstallments(sellItemsData, activeTab as InstallmentStatus).filter(
    (item) =>
          ...
          <header className="h-16 border-b border-slate-200 dark:border-slate-700/50 bg-white/80 dark:bg-slate-800/30 backdrop-blur-md flex items-center justify-between px-4 md:px-6">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={() => {dispatch(setIsAuthenticated(false)), navigate("/auth")}}>Sign Out</Button>
              </div>
            </div>
          </header>
